...
export const submitResponsesAndDiagnose = async (req, res, next) => {
  try {
    ...
    // صلي على رسول الله بصوت رامز جلال في فيلم شبه منحرف
    // اللهم نويت اصلي ركعتين سنة العشاء الله اكبر
    // لا ننسى حسبي الله ونعم الوكيل
    ...
    if (discoid === 1 && subacute === 0 && acute === 0) {
      skinLupusScore = 3;
    } else if (discoid === 0 && subacute === 1 && acute === 0) {
      skinLupusScore = 1;
    } else if (discoid === 0 && subacute === 0 && acute === 1) {
      skinLupusScore = 2;
    } else if (discoid === 1 && subacute === 1 && acute === 0) {
      skinLupusScore = 3; // كله كله قوم وزعلك انت قوم بصوت علي ربيع
    } else if (discoid === 1 && subacute === 0 && acute === 1) {
      skinLupusScore = 2;
    ...
    const encodedInputs = [
      ...
      encode(answerMap[16]),
      encode(answerMap[17]),
      encode(answerMap[18], "High"), // منك لله مش هقول غير كده
      encode(answerMap[19]),
      renalBiopsyClass,
      encode(answerMap[21]),
      encode(answerMap[22]),
    ...
    const aiResponse = await axios.post("https://lupira-ai-dnmi.onrender.com/predict", {
      ...
      anti_cardiolipin_anitbody: encodedInputs[18],  //كتبتها غلط عشان اعرف اعمل تيستينج عشان حسن كاتبها كده
